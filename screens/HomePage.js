		...
		const getJobsMatchingUserExperiences = async () => {
			try {
				...
				const matchingJobs = allJobs.filter((job) =>
					jobNamesArray.some((jobName) =>
						job.job_name.toLowerCase().includes(jobName)
					)
				);
			...
			<ScrollView>
				...
				<View style={styles.searchContainer}>
					...
					<TouchableOpacity
						...
						onPress={() => {
							navigation.navigate("SearchResultFromHomePage", {
								searchTerm,
							});
						}}
					>
